...
export function iterateOverInput(input: string[]): number {
  return input.reduce((acc, curr) => {
    ...
    return acc +
      calculateScore(opponent as OpponentMove, player as GameOutcomes);
  }, 0);
}
...
Deno.test("it should match the first example round", () => {
  ...
  assert(
    Object.keys(opponentMoves).some((key) => key === opponent),
    "Opponent move is not valid",
  );
  assert(
    Object.keys(playerMoves).some((key) => key === outcome),
    "Player move is not valid",
  );

  const result = calculateScore(
    opponent as OpponentMove,
...
Deno.test("it should match the third example round", () => {
  ...
  assert(
    Object.keys(GameOutcomes).some((key) => key === outcome),
    "Player move is not valid",
  );
